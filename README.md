...
## 🧭 项目定位：编排为主的工作流 + 局部 Agent 能力

**一句话**：这是一个以 **LangGraph 确定性编排（Workflow）为主干**、在局部具备 **Agent（自主决策）** 能力的混合型系统——**不是典型意义的"自主 Agent 系统"**。

### 为什么说"编排为主"

系统的主流程是一条**代码写死的推理图**，模型不能发明路径：

```text
intent → memory → analysis → tool_use → research_plan → evidence_router
→ retrieve → evidence_judge →(证据不足→query_rewrite→再检索, 有界)
→ reason → debate → consensus_agent → validate →(反思循环, 有界)
→ compliance(合规审计) → human_review(HITL医生复核) → generate_report
```

- 节点顺序、条件路由（意图分诊 / 反思回环 / 检索循环）全部由 `clinical_graph.py` 预先定义；
- 每个"专家"本质是 **"固定 Prompt + LLM + 结构化输出"的节点**，没有独立工具、目标循环或记忆；
- 模型只能在图给定的边里做有限选择（"证据够不够、要不要重查""校验是否通过、要不要反思"）。

### 局部 Agent 能力（真正的自主成分）

| 成分 | 说明 |
|---|---|
| **工具调用 Agent**（`tool_use` 节点） | 真正的 function-calling 循环：LLM 自主决定调用 18 个脑卒中医疗工具、按 Level 优先级、最多 N 轮、失败时规则兜底 |
| **Agentic RAG 闭环** | 检索不是一次性：规划检索任务 → 路由证据源 → 检索 → 评估证据充分性 → 不足则改写查询再查（默认最多两轮） |

### 为什么这么设计（医疗场景的正确取舍）

医疗 CDSS **刻意牺牲 Agent 的"自由"，换取可控性与可追责**：

- 固定步骤 → 思考链完整可展示、可逐节点校验、可合规审计；
- 反思/检索循环**有界** → 不会失控烧 token；
- 报告生成前强制医生复核（HITL interrupt/resume）→ 可追责；
- 全程版本快照（模型/知识库/Prompt 随报告留痕）→ 可追溯。

**对外介绍建议**：可称"基于 LangGraph 的多智能体协作式临床决策系统"（reason/debate/consensus 的多角色协作是真实的），但内部本质是受控编排工作流。
...
系统在推理完成后基于「问题 + AI 回答」自动生成对话标题（如「房颤患者溶栓评估」「急性脑梗死」），用简洁医学术语概括对话主题；对话列表过滤空内容记录，新建对话占位符在刷新时稳定保留；被范围拦截的无关问题（如「你好」）也会创建并保存对话，确保每条消息都有迹可循。
...
> 📌 **定位说明**：本节所述"多智能体"指**多专家角色（全科医生/神经专科/临床药师）的协作推理**，其实现为 LangGraph 确定性编排（详见上文「项目定位」章节）——角色分工与协作流程是真实的多智能体模式，但流程本身受控、可追溯。
...
- 新对话在**首次发送时**由后端创建（`talkId` 通过 SSE 事件返回前端，前端据此更新列表占位符）
- 与脑卒中临床无关的问题（如「你好」等纯问候语）由 `QuestionScopeGuard` 拦截，**同样会创建对话**并保存「问题 + 范围提示」，侧边栏可见（标题为「问题范围提示」），避免"发了消息却没对话记录"
- 对话列表只展示**有实际内容**的记录（空内容对话不显示），标题由 AI 基于「问题 + 回答」自动生成
...
| [docs/rag-retrieval-enhancements.md](docs/rag-retrieval-enhancements.md) | ★ **RAG 检索提升方法总结** — Multi-Collection 隔离、chunk 级元数据、垃圾过滤、重分桶、Medical Evidence Score、PICO 双语查询、LLM 语义分类、RAGAS 基准与指标 |
...
### v2.10.0 (2026-08-15) — 功能全面扩展

- ✅ **MDT 会诊纪要导出**：一键导出正式多学科会诊记录（病例资料 + 会诊过程全文 + 结论 + 审核签名栏），打印/另存 PDF
- ✅ **LLM 成本与用量看板**：langchain callback 按真实 token_usage 记账，思考面板展示 tokens/调用次数/估算成本
- ✅ **会诊过程时间轴**：思考事件记录起止时间戳，面板顶部泳道图分色展示各阶段耗时（流程/专家会诊/校验报告）
- ✅ **信息缺口追问闭环**：会诊结束自动列出关键信息缺口（ask_doctor 事件），前端"补充并重新会诊"卡片一键继续
- ✅ **化验单拍照解析填表**：qwen-vl 结构化提取血压/血糖/血小板/INR，绿道表单自动回填
- ✅ **医疗工具扩容 8 → 16**：新增 ASPECTS、CHA₂DS₂-VASc、HAS-BLED、洼田吞咽筛查、rt-PA 24h 监测清单、DVT/压疮预防清单、出血转化风险分层、出院随访计划
- ✅ **证据页码定位跳转**：引用携带 p.N 页码，点击预览自动滚动到对应页
- ✅ **绿道质控看板**：DNT 中位数/4.5h 窗内率/NIHSS 均值/关键字段完整率/风险信号/14 天趋势
- ✅ **知识库管理界面**：上传/删除指南 PDF 自动后台重建向量库（任务轮询），展示文献/分块/collection 统计
- ✅ **检索质量实时看板**：retrieve/evidence_judge 输出结构化指标（证据数/来源分布/质量分 chips）
- ✅ **更多体验**：语音录入（Web Speech）、Actuator 健康/指标端点、共识节点模型可配置（CONSENSUS_MODEL）
- ✅ **思维链持久化**：每轮推理过程/专家意见/用量随对话入库（V3 迁移），刷新后历史对话可随时重新打开思维链
- ✅ **修复**：雪花 ID 超出 JS 安全整数导致前端精度丢失（表现为"刷新后问答消失"）——Boot 4 REST 层全局 Long→String 序列化；Redis 增加持久化卷+AOF（容器重启不丢会话）
...
- ✅ **修复**：Boot 4 下 Flyway 自动配置移至独立模块，显式引入 `spring-boot-starter-flyway` + `flyway-mysql`，恢复 `db/migration` 自动执行（修复全新数据卷部署时 `talk.patient_id` 缺失导致的 500）
